...
import { ChatMessage, dotCanvas, formatFileSize, Phase } from "./utils";
...
export default function Home() {
    ...
    try {
      ...
      if (data.summary) {
        ...
        await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            documentText: data.extractedText || data.summary,
            query: "",
          }),
        });
        setIsProcessed(true);
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...prev,
        { role: "ai", text: data.answer || `Error: ${data.error}` },
      ]);
    } catch {
      setChatHistory((prev) => [
        ...prev,
        { role: "ai", text: "Failed to communicate with engine." },
      ]);
    } finally {
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F3] p-6 md:p-10">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex flex-col flex-wrap items-baseline gap-x-3 gap-y-1">
          <h1 className="text-[28px] font-semibold text-[#1C1B19]">
            Summariza
          </h1>
          <p className="text-sm text-[#6B6457]">
            Upload a document, read the essentials, then ask it anything.
          </p>
        </header>
          ...
          <div className="flex flex-col rounded-2xl border border-[#DDD6C7] bg-white p-6 lg:col-span-2 lg:h-[calc(100vh-200px)] lg:overflow-y-auto">
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#6B6457]">
                    One document at a time · PDF only
                  </p>
                </div>
              </label>
            )}
            ...
            {phase === "preview" && (
              ...
              >
                <div>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3 text-sm">
                    ...
                    <button
                      ...
                      className="shrink-0 text-xs font-medium text-[#A8752E] hover:underline"
                    >
                ...
                <button
                  type="submit"
                  disabled={!file}
                  className="w-full rounded-lg bg-[#1C1B19] py-2.5 text-sm font-medium text-white transition hover:bg-[#33312C] disabled:cursor-not-allowed disabled:bg-[#DDD6C7]"
                >
                  Summarize document
                </button>
              </form>
            ...
            {phase === "processing" && (
              <div className="flex grow flex-col justify-center gap-6">
                ...
                <div className="rounded-xl border border-[#DDD6C7] bg-[#FAF8F3] px-6 py-10 text-center flex justify-center items-center flex-col gap-4">
                  <p className="mb-4 text-sm text-[#1C1B19]">
            ...
            {phase === "done" && (
              <div className="flex grow flex-col gap-4">
                ...
                <div className="rounded-xl border border-[#DDD6C7] bg-white p-5">
                  ...
                  <p className="max-h-76 overflow-y-auto whitespace-pre-wrap pr-1 text-[14px] leading-relaxed text-[#332F28]">
                    {summary}
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col rounded-2xl border border-[#DDD6C7] bg-white lg:col-span-3 lg:h-[calc(100vh-200px)]">
            <div className="shrink-0 border-b border-[#DDD6C7] px-6 py-4">
              ...
              <p className="mt-0.5 text-xs text-[#6B6457]">
                {isProcessed
                  ? "Answers are grounded in the file you uploaded."
              ...
              {!isProcessed ? (
                <div className="flex h-full min-h-55 flex-col items-center justify-center text-center">
                  <p className="text-sm text-[#A39C8A]">
                    Upload a document on the left to start a conversation about
                    it.
                  </p>
                </div>
                ...
                <div className="flex h-full min-h-55 flex-col items-center justify-center text-center">
                  <p className="text-sm text-[#6B6457]">
                    Your document is ready. Try asking:
                  </p>
                  <p className="mt-1 text-sm font-medium text-[#A8752E]">
                    “E.g What does page 2 cover?”
                  </p>
                </div>
              ) : (
